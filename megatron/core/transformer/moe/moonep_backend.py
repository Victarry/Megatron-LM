...
def _import_moonep():
    try:
        ...
        raise RuntimeError(
            "BalancedMoELayer requires the pinned Victarry/MoonEP fork " "and its CUDA extension"
        ) from exc
    return moonep
...
class _DenseRouteToMoonEP(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, probs: torch.Tensor, routing_map: torch.Tensor, topk: int
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Pack a dense route into MoonEP's fixed top-k representation."""
        ...
        tokens_per_expert = torch.bincount(expert_ids.reshape(-1), minlength=num_experts).to(
            torch.int32
        )
    ...
    def backward(ctx, grad_route_weights, _grad_expert_ids, _grad_tokens_per_expert):
        """Scatter top-k route gradients back into the dense probability tensor."""
        ...
        if grad_route_weights is not None:
            ...
            grad_probs.scatter_add_(1, expert_ids, grad_route_weights.masked_fill(~valid, 0.0))
...
class MoonEPRuntimeKey:
    """Static shape and launch configuration identifying a reusable runtime."""
...
class MoonEPRuntime:
    ...
    def __init__(self, key: MoonEPRuntimeKey, group: ProcessGroup, ep_size: int) -> None:
    ...
    def dispatch_and_prefetch(
        ...
        """Dispatch routed tokens and prefetch the selected expert projections."""
        ...
        with torch.no_grad():
            dispatched_hidden, dispatched_weights, cu_seqlens, plan, _ = self.buffer.dispatch(
                hidden,
                route_weights,
                expert_ids,
                tokens_per_expert,
    ...
    def combine(
        self, plan, hidden: torch.Tensor, route_weights: torch.Tensor | None = None
    ) -> tuple[torch.Tensor, torch.Tensor | None]:
        """Combine dispatched token rows and optional route-weight gradients."""
    ...
    def redispatch(self, plan, hidden: torch.Tensor) -> torch.Tensor:
        """Dispatch output gradients with an existing forward plan."""

        with torch.no_grad():
            dispatched, _, _, _, event = self.buffer.dispatch(
                hidden, plan=plan, async_finish=True, zero_copy=False
            )
        self._wait(event)
        return dispatched

    def reduce_grads(self, plan, table) -> None:
        """Reduce replicated FP32 expert gradients to their home rows."""

        with torch.no_grad():
            event = self.buffer.reduce_grads(
                plan, table.full_grads, table.reduce_buffers, async_finish=True
            )
        self._wait(event)

    def close(self) -> None:
        """Destroy the MoonEP buffer exactly once."""
...
class MoonEPRuntimeRegistry:
    ...
    def get(
        ...
        *,
        ...
        """Return the runtime for a static communication shape."""
    ...
    def register_resource(cls, resource) -> None:
        """Register a VMM-backed resource for ordered process-group teardown."""
    ...
    def close_all(cls) -> None:
        """Close registered VMM resources and communication runtimes."""
...
class _MoonEPDispatchAutograd(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, hidden, route_weights, dispatched_hidden, dispatched_route_weights, runtime, plan
    ):
        """Attach explicit communication backward semantics to dispatched tensors."""
    ...
    def backward(ctx, grad_hidden, grad_route_weights):
        """Combine hidden and route gradients with the saved plan."""

        if grad_hidden is None:
            raise RuntimeError("MoonEP dispatch backward requires hidden-state gradients")
        grad_input, grad_probs = ctx.runtime.combine(ctx.plan, grad_hidden, grad_route_weights)
...
class _MoonEPCombineAutograd(torch.autograd.Function):
    ...
    def forward(ctx, expert_output, combined_output, runtime, plan):
        """Attach redispatch backward semantics to the combined output."""
    ...
    def backward(ctx, grad_output):
        """Redispatch output gradients with the saved plan."""
...
class _MoonEPWeightGradBarrier(torch.autograd.Function):
    ...
    def forward(ctx, hidden, backend, runtime, plan, *home_parameters):
        """Keep home parameters in autograd while forwarding hidden unchanged."""
    ...
    def backward(ctx, grad_hidden):
        """Fold FP32 VMM gradients before releasing DDP parameter hooks."""
...
class MoonEPGroupedMLP:
    ...
    def _local_projection_weights(self) -> list[list[torch.Tensor]]:
        def weights(layer) -> list[torch.Tensor]:
            if getattr(layer, "single_grouped_weight", False):
                raise ValueError("MoonEP requires per-expert TE grouped weights")
            return [getattr(layer, f"weight{index}") for index in range(self.experts_per_rank)]

        return [weights(self.experts.linear_fc1), weights(self.experts.linear_fc2)]

    @property
    def home_parameters(self) -> tuple[torch.Tensor, ...]:
        """Return checkpoint-owned FC1 and FC2 parameters in projection order."""

        return tuple(
            weight for projection in self._local_projection_weights() for weight in projection
        )
    ...
    def _build_ops(self):
        ...
        activation_kwargs = {}
        if "glu_interleave_size" in inspect.signature(te.pytorch.ops.ScaledSwiGLU).parameters:
    ...
    def refresh_shadow(self) -> None:
        """Refresh local VMM rows from gathered home parameters."""
    ...
    def begin(self, plan) -> None:
        """Acquire the single outstanding training-plan lease."""
    ...
    def forward(
        ...
        """Run the private TE graph over MoonEP VM-group token rows."""
        ...
        return self.ops(guarded_hidden, tokens_per_group, dispatched_probs, tokens_per_group)

    def fold_weight_grads(self, runtime: MoonEPRuntime, plan) -> None:
        """Reduce FP32 VMM wgrad and accumulate local rows into home main_grad."""
        ...
        for full_grad, local_weights in zip(self.table.full_grads, local_projection_weights):
    ...
    def close(self) -> None:
        """Release the private TE graph and its VMM table."""
...
class MoonEPDataPlane:
    ...
    def __init__(
        ...
        self.grouped_mlp = MoonEPGroupedMLP(experts, config, group, ep_rank, ep_size, num_experts)
    ...
    def forward(
        ...
        *,
        ...
        """Execute the complete MoonEP communication and expert data plane."""
        ...
        if refresh_shadow or self.grouped_mlp.table is None or not torch.is_grad_enabled():
            self.grouped_mlp.refresh_shadow()

        dispatched_hidden, dispatched_probs, cu_seqlens, plan = runtime.dispatch_and_prefetch(
            hidden_2d,
            route_weights,
            expert_ids,
            tokens_per_expert,
        ...
        dispatched_hidden, dispatched_probs = _MoonEPDispatchAutograd.apply(
            hidden_2d, route_weights, dispatched_hidden, dispatched_probs, runtime, plan
        )
