...
def _benchmark_phase(fn, *, warmup=5, iterations=20):
    ...
    event_ms = torch.tensor(sum(event_latencies) / iterations, dtype=torch.float64, device="cuda")
    wall_ms = torch.tensor(sum(wall_latencies) / iterations, dtype=torch.float64, device="cuda")
...
def test_moonep_route_adapter_dispatch_combine_forward_backward_parity():
    ...
    try:
        ...
        expert_ids = torch.stack(
            (token_ids % num_experts, (token_ids * 7 + 3) % num_experts), dim=1
        )
        ...
        route_weights, topk_ids, tokens_per_expert = adapt_mcore_route(probs, routing_map, topk)
        ...
        group_experts = torch.arange(num_experts + num_spares, dtype=torch.int32, device=device)
        group_experts[num_experts:] = plan.experts_to_copy[rank].clamp_min(0)
        starts = torch.cat((torch.zeros(1, dtype=cu_seqlens.dtype, device=device), cu_seqlens[:-1]))
        ...
        output_grad = torch.randn(
            actual_output.shape, dtype=torch.bfloat16, device=device, generator=generator
        )
        ...
        grad_hidden, grad_route_weights, _ = buffer.combine(
            plan=plan, hidden_nvsh=grad_dispatched, route_weights_nvs=grad_dispatched_weights
        )
...
def test_moonep_vmm_shadow_prefetch_and_fanout_fp32_grad_reduce():
    ...
    try:
        ...
        home_weight = torch.full(projection_shape, rank + 1, dtype=torch.bfloat16, device=device)
        table.refresh_home_weights([[home_weight]])

        buffer = moonep.Buffer(
            S=128, H=128, K=1, E=num_experts, B=num_spares, num_ep_ranks=world_size, group=group
        )
        ...
        torch.testing.assert_close(
            table.full_weights[0][num_experts], table.full_weights[0][0], rtol=0, atol=0
        )
        ...
        torch.testing.assert_close(
            local_grad, torch.full_like(local_grad, expected), rtol=0, atol=0
        )
        torch.testing.assert_close(
...
@pytest.mark.parametrize(
    ...
    [("representative_fc1_proxy", (4096, 2048)), ("representative_fc2_proxy", (2048, 4096))],
)
def test_moonep_data_plane_latency_and_actual_payload_bandwidth(case_name, projection_shape):
    del case_name
    moonep = _require_moonep()
    Utils.initialize_model_parallel(tensor_model_parallel_size=1, expert_model_parallel_size=4)
    buffer = None
    ...
    try:
        ...
        local_weights = [
            torch.full(
                projection_shape, rank * num_spares + index + 1, dtype=torch.bfloat16, device=device
            )
        ...
        ids = (torch.arange(num_tokens, dtype=torch.int32, device=device) % num_experts).view(-1, 1)
        counts = torch.bincount(ids.flatten(), minlength=num_experts).to(torch.int32)
        ...
        for destination in range(world_size):
            ...
            plan.experts_to_copy[destination].copy_(
                torch.arange(
                    owner * num_spares, (owner + 1) * num_spares, dtype=torch.int32, device=device
                )
            )
        ...
        def dispatch_phase():
            result = buffer.dispatch(
                hidden, weights, ids, counts, async_finish=True, zero_copy=False
            )
        ...
        _print_moonep_benchmark(
            "planning_token_dispatch_forward", event_ms, wall_ms, measured_payload.dispatch
        )
        ...
        def combine_phase():
            result = buffer.combine(
                plan=measured_plan, hidden_nvsh=dispatched, async_finish=True, zero_copy=False
            )
            return result, result[-1]
        ...
        def redispatch_phase():
            result = buffer.dispatch(hidden, plan=measured_plan, async_finish=True, zero_copy=False)
            return result, result[-1]

        redispatch_result, event_ms, wall_ms = _benchmark_phase(redispatch_phase)
        grad_dispatched = redispatch_result[0]
        _print_moonep_benchmark(
            "output_grad_redispatch_backward", event_ms, wall_ms, measured_payload.redispatch
        )
        ...
        _print_moonep_benchmark(
            "expert_weight_grad_reduce_backward", event_ms, wall_ms, payload.grad_reduce
        )
    finally:
        if table is not None:
            table.close()
...
def test_balanced_moe_moonep_full_layer_parity_refresh_and_state_dict():
    ...
    try:
        ...
        for iteration in range(2):
            ...
            output_grad = torch.randn(
                hidden.shape, dtype=torch.bfloat16, device="cuda", generator=generator
            )
            ...
            for baseline_param, balanced_param in zip(
                ...
                torch.testing.assert_close(_grad(balanced_param), _grad(baseline_param).float())
            for baseline_param, balanced_param in zip(
        ...
        eval_hidden = torch.randn(
            8, 4, 512, dtype=torch.bfloat16, device="cuda", generator=generator
        )
        padding_mask = torch.zeros(4, 8, dtype=torch.bool, device="cuda")
        padding_mask[:, -1] = True
        with torch.no_grad():
            for _ in range(2):
                ...
                torch.testing.assert_close(balanced_output, baseline_output)
                assert balanced.data_plane.grouped_mlp._active_plan is None
    finally:
        Utils.destroy_model_parallel()
